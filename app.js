/* La Linea — Inline Character Prototype — continuity fix + arrows */
(() => {
  'use strict';
  ...
  // omino inline
  let GUY_X = Math.round(W*0.28);           // ora è mobile
  const GUY_W = 96;
  const STROKE = 8;
  const GUY_MIN_X = 40;
  const GUY_MAX_X = W - 40;

  // salto come deformazione della linea
  ...
  function baselineAt(x){
    ...
    for(const o of obst){
      ...
      if(o.type==='step'){
        ...
        const u=(x-L)/o.w; y = baseY0 - o.h*Math.sin(u*Math.PI);
      }
    }
    return y;
  }

  // profilo inline — FIX: si aggancia alla baseline a sinistra e a destra
  function drawInlineMan(gx, lift){
    const start = gx - GUY_W/2, end = gx + GUY_W/2;
    const yL = baselineAt(start), yR = baselineAt(end);
    if(yL===null || yR===null){ // se proprio sotto ai bordi c'è un gap, la linea si spezza
      running=false; 
      return {start, end, broke:true};
    }

    const s=1, h=120*s, w=56*s, arm=46*s;
    const x0 = gx - 10*s; // piccolo offset per forma

    // piede a L dalla baseline sinistra
    ctx.lineTo(start, yL);
    const yTop = (yL + yR)/2 + lift; // usiamo media per seguire pendenze → continuità
    ...
    // rientro al fianco dx e aggancio alla baseline destra (yR)
    ...
    // bocca (decorativa, non influisce sulla continuità)
  ...
  function drawWorldInline(){
    ctx.lineWidth=STROKE; ctx.lineCap='round'; ctx.lineJoin='round'; ctx.strokeStyle=FG;
    ctx.beginPath();

    const step=8, guyStart=GUY_X-GUY_W/2, guyEnd=GUY_X+GUY_W/2;
    let moved=false;

    // fino a prima dell’omino
    ...
    if(yCheckL===null || yCheckR===null){ running=false; }
    else{
  ...
  function tick(){
    if(running){
      ...
      }else if(hand.show){ hand.x -= speed*.8; }

      // movimento orizzontale omino
      if(!holdStill){
      ...
      // regola: se un gap attraversa la porzione dell’omino ⇒ game over
    ...
    const el=document.getElementById('score'); if(el) el.textContent=score.toString();
  ...
  // touch: bordo sinistro/destra = muovi, centro = salta, due dita = ferma
  const touch = document.getElementById('touch') || cvs;
  let touchIdL=null, touchIdR=null;
  function whichZone(x){
  ...
  touch.addEventListener('pointerdown', e=>{
    ...
    if(e.isPrimary && e.pointerType!=='mouse' && e.width>0 && e.height>0){
      // se tocco già presente e ne arriva un secondo ⇒ holdStill
      if(moveLeft||moveRight||inputHeldJump) holdStill=true;
    }
    if(zone==='left'){ moveLeft=true; }
    else if(zone==='right'){ moveRight=true; }
    else { pressDownJump(); setTimeout(releaseJump,0); } // tap = salto singolo
  });
  ['pointerup','pointercancel','pointerleave'].forEach(ev=>{
    touch.addEventListener(ev, ()=>{
      moveLeft=false; moveRight=false; holdStill=false; releaseJump();
    });
  });
